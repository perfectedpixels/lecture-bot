...
class ConceptExtractor:
    """
    Uses Claude to extract and tag concepts from lecture segments.
    """
...
Extract up to {max_concepts} most important concepts. Focus on substantive topics, not filler words."""
        ...
        try:
            content = call_claude(prompt, max_tokens=1000, temperature=0.3, model=self.model_id)
    ...
    def determine_primary_category(self, concepts: List[ConceptTag]) -> str:
        """Determine the primary category using weighted voting by confidence."""
        if not concepts:
            return "General"
    ...
    def to_metadata(self, tagged_segment: TaggedSegment) -> Dict[str, str]:
        """Convert tagged segment to metadata format."""
